...
class ApiSmHomeWorkController extends Controller
{
    ...
    public function saas_studentHomework(Request $request, $school_id, $user_id, $record_id)
    {
        try {
            ...
            foreach ($homeworkLists as $s_homework) {
                $student_result = SmHomeworkStudent::where('homework_id', $s_homework->id)
                    ->where('student_id', $student->id)
                    ->first();
    ...
    public function saas_studentUploadHomework(Request $request, $school_id)
    {
        ...
        try {
            ...
            $content->academic_id = SmAcademicYear::API_ACADEMIC_YEAR($school_id);
            $result = $content->save();
            ...
            $notification->academic_id = SmAcademicYear::API_ACADEMIC_YEAR($school_id);
            $notification->save();
