...
class WLSM_M_Session {
	...
	public static function get_pre_session( $current_session_id ) {
		global $wpdb;
		$current_session = $wpdb->get_row( $wpdb->prepare( 'SELECT ss.end_date FROM ' . WLSM_SESSIONS . ' as ss 
			WHERE ID = %d', $current_session_id ) );

		$previous_session = $wpdb->get_row( $wpdb->prepare( 'SELECT ss.ID, ss.label FROM ' . WLSM_SESSIONS . ' as ss 
		WHERE ss.end_date < %s ORDER BY ss.end_date DESC LIMIT 1', $current_session->end_date ) );
