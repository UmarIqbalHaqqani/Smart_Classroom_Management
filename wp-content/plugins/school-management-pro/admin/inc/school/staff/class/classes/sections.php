			...
			<div class="card-header">
				<span class="float-left h5">
					...
					printf(
						wp_kses(
							...
							__( '<span class="text-secondary">Class:</span> %s', 'school-management' ),
							array(
				...
				<span class="float-md-right">
					<a href="<?php echo esc_url( $page_url ); ?>" class="btn btn-sm btn-info">
						...
						<?php esc_html_e( 'View Classes', 'school-management' ); ?>
					</a>
				</span>
			</div>
			
				<div class="row<?php if ( $section ) { echo ' justify-content-md-center'; } ?>">
					<?php if ( ! $section ) { ?>
					<div class="col-md-7">
						<h2 class="h4 border-bottom pb-2">
							<i class="fas fa-layer-group text-primary"></i>
							<?php esc_html_e( 'Class Sections', 'school-management' ); ?>
						</h2>
					...
					<div class="col-md-5">
							...
							<h2 class="h4 border-bottom pb-2 wlsm-page-heading">
								...
								printf(
									wp_kses(
										...
										__( 'Edit Section: <span class="text-secondary">%s</span>', 'school-management' ),
										array(
											'span' => array( 'class' => array() )
										)
									),
								...
								?>
								<a href="<?php echo esc_url( $page_url . '&action=sections&id=' . $class->class_id ); ?>" class="float-md-right btn btn-sm btn-outline-primary">
									<i class="fas fa-plus-square"></i>
									<?php esc_html_e( 'Add New Section', 'school-management' ); ?>
								</a>
								<?php } else { ?>
						...
						<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-section-form">
							...
							<div class="form-group">
								<label for="wlsm_section_label" class="font-weight-bold"><?php esc_html_e( 'Section', 'school-management' ); ?>:</label>
								<input type="text" name="label" class="form-control" id="wlsm_section_label" placeholder="<?php esc_attr_e( 'Enter section', 'school-management' ); ?>" value="<?php echo esc_attr( WLSM_M_Staff_Class::get_section_label_text( $label ) ); ?>">
							</div>

							<?php if ( $section && ( $section_id == $class->default_section_id ) ) { ?>
							<div class="text-secondary mb-1">
								<?php esc_html_e( 'Currently set to default section.', 'school-management' ); ?>
							</div>
							<?php } else { ?>
							<div class="form-group">
								...
								<label class="ml-4 mb-1 form-check-label wlsm-font-bold text-dark" for="wlsm_is_default">
									<?php esc_html_e( 'Set as default section?', 'school-management' ); ?>
								</label>
							</div>
							<?php } ?>
								...
								<span class="float-md-right">
									<button type="submit" class="btn btn-sm btn-primary" id="wlsm-save-section-btn">
										...
										if ( $section ) {
											...
											<?php
											esc_html_e( 'Update Section', 'school-management' );
										} else {
											?>
											<i class="fas fa-plus-square"></i>&nbsp;
											<?php
											esc_html_e( 'Add New Section', 'school-management' );
										}
										?>
									</button>
								</span>
