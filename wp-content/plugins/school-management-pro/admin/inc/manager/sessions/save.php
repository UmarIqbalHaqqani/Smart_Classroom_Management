			...
			<div class="col-md-12">
				<div class="wlsm-main-header card col wlsm-page-heading-box">
					...
					<?php if ( $session ) { ?>
						...
						printf(
							wp_kses(
								...
								__( 'Edit Session: <span class="text-secondary">%s</span>', 'school-management' ),
								array(
					...
					<?php } else { ?>
						...
						<?php esc_html_e( 'Add New Session', 'school-management' ); ?>
					<?php } ?>
					</h1>
				</div>
			</div>
					...
					<div class="card-body">
						<form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post" id="wlsm-save-session-form">
							...
							<div class="form-group">
								...
								<input type="text" name="label" class="form-control" id="wlsm_label" placeholder="<?php esc_attr_e( 'Enter label', 'school-management' ); ?>" value="<?php echo esc_attr( WLSM_M_Session::get_label_text( $label ) ); ?>">
							</div>
							...
							<div class="form-group">
								<label for="wlsm_end_date" class="font-weight-bold">
									<span class="wlsm-important">*</span> <?php esc_html_e( 'End Date', 'school-management' ); ?>:
								</label>
								...
								<span class="float-md-right">
									<button type="submit" class="btn btn-sm btn-primary" id="wlsm-save-session-btn">
										...
										if ( $session ) {
											...
											<?php
											esc_html_e( 'Update Session', 'school-management' );
										} else {
											?>
											<i class="fas fa-plus-square"></i>&nbsp;
											<?php
											esc_html_e( 'Add New Session', 'school-management' );
										}
										?>
									</button>
								</span>
