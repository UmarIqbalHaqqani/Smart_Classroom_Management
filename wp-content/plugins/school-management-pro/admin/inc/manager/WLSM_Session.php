...
class WLSM_Session {
	...
	public static function save() {
		...
		try {
			...
			if ( empty( $label ) ) {
				$errors['label'] = esc_html__( 'Please provide session label.', 'school-management' );
			} else {
				if ( strlen( $label ) > 191 ) {
					$errors['label'] = esc_html__( 'Maximum length cannot exceed 191 characterss.', 'school-management' );
				}
			}

			if ( $start_date >= $end_date ) {
				$errors['start_date'] = esc_html__( 'Session start date must be lower than end date.', 'school-management' );
			}

			if ( empty( $start_date ) ) {
				$errors['start_date'] = esc_html__( 'Please provide start date of a session.', 'school-management' );
			} else {
				$start_date = $start_date->format( 'Y-m-d' );
			}

			if ( empty( $end_date ) ) {
				$errors['end_date'] = esc_html__( 'Please provide end date of a session.', 'school-management' );
			} else {
			...
			if ( $session_exist ) {
				$errors['label'] = esc_html__( 'Session already exists with this label, start date and end date.', 'school-management' );
			}
			// End validation.
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				if ( $session_id ) {
					...
					$message = esc_html__( 'Session updated successfully.', 'school-management' );
					$reset   = false;
				} else {
					$data['created_at'] = current_time( 'Y-m-d H:i:s' );

					$success = $wpdb->insert( WLSM_SESSIONS, $data );
					$message = esc_html__( 'Session added successfully.', 'school-management' );
					$reset   = true;
				}
	...
	public static function delete() {
		...
		try {
			...
			$message = esc_html__( 'Session deleted successfully.', 'school-management' );
