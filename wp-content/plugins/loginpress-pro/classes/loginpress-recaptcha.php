...
if ( ! class_exists( 'LoginPress_Recaptcha' ) ) {
	...
	class LoginPress_Recaptcha {
		...
		private function hooks() {
			...
			if ( isset( $_POST['pp-lf-login-nonce'] ) || isset( $_POST['pp-registration-nonce'] ) ) { // @codingStandardsIgnoreLine.
				return;
			}

			if ( 'off' === $cap_permission || empty( $cap_site ) || empty( $cap_secret ) ) {
				return;
			}
			...
			if ( class_exists( 'woocommerce' ) ) {
				...
				if ( $woo_login_enable ) {
					...
					add_filter( 'authenticate', array( $this, 'loginpress_recaptcha_auth' ), 10, 3 );
					...
					<script>
						document.addEventListener("DOMContentLoaded", function() {
							grecaptcha.ready(function() {
								grecaptcha.execute('<?php echo esc_attr( $cap_site_v3 ); ?>', {action: 'loginpage'}).then(function(token) {
		 ...
		 *
		 * @param  array $errors The Error.
		 * @return array $errors The Error.
		 *
		 * @version 2.1.2
		 */
		public function loginpress_recaptcha_lostpassword_auth( $errors ) {
