...
if(!empty($wppm_email_notificatins)){
  foreach ($wppm_email_notificatins as $key=>$val) :
    if($val['type']=='change_project_status'){
      ...
      $email_addresses = apply_filters('wppm_en_change_status_email_addresses',$email_addresses,$val,$project_id);
      ...
      do_action('wppm_after_project_change_status_mail',$project_id,$val);
    }
  endforeach;
}
