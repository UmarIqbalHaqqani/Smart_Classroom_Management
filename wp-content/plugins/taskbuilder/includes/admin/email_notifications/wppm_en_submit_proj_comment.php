...
$proj_id = intval(sanitize_text_field($proj_id));
...
foreach ($wppm_email_notificatins as $key=>$val) :
  if($val['type']=='new_proj_discussion'){
    $subject  = $wppmfunction->replace_macro(stripslashes($val['subject']), $proj_id);
    $body  = $wppmfunction->replace_macro(stripslashes($val['body']),$proj_id);
      ...
      foreach ($recipients as $recipient) {
          if(is_numeric($recipient)){
            ...
            switch ($recipient) {
                ...
                $email_addresses = array_merge($email_addresses,$wppmfunction->get_previously_assigned_users($proj_id));
                break;
            }
          }
      }
    ...
    $email_addresses = apply_filters('wppm_en_proj_assign_users_email_addresses',$email_addresses,$val,$proj_id);
    ...
    do_action('wppm_after_submit_proj_comment_mail',$proj_id,$val);
  }
endforeach;